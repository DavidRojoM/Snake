# Snake LIBGDX

## 🔴Integrantes del grupo.

* Daniel Delgado
* David Rojo
* Alejandro Gómez
* Javier Sánchez
* Miguel Angel López
* Daniel Mulero
* * *
## 🔴Reparto de tareas.

* **Daniel Delgado**
  * Diseño gráfico
  * Búsqueda de efectos de sonido
  * Tareas de programación
  * Corrección de errores
  * Diagrama de objetos
  * Investigación en documentación externa.
* **David Rojo**
  * Coordinación de grupo
  * Creación de repositorio
  * Tareas de programación
  * Corrección de errores
  * Diagrama de objetos
  * Investigación en documentación externa
  * Realización y mantenimiento del README.md.
* **Alejandro Gómez**
  * Diseño de diagramas de estado.
* **Javier Sánchez**
  * Diseño de diagrama UML
  * Diagrama de casos de uso
  * Diagrama de objetos.
* **Miguel Angel López**
  * Diagrama de actividad
  * Diagrama de objetos.
* **Daniel Mulero**
  * Diagrama de actividad
  * Diagrama de objetos.
* * *
## 🔴Propuesta de diseño.

<ul>
  <li>El aspecto gráfico de las imagenes splash de principio y fin del juego se inspirarán en los pixelart de los juegos retro</li>
  <li>El fondo de pantalla será un pixelart verde, el cual imitará a un césped</li>
  <li>La serpiente será de color rojo para aumentar el contraste con el fondo</li>
  <li>La temática de los sonidos tambien es de estilo retro</li>
  <li>Hemos decidido que el sonido que se reproduce cada vez que la serpiente se mueva sea discreto para no molestar al jugador</li>
</ul>

* * *

## 🔴Actualizaciones.

### 2021-01-12.
* - [x] Inicializar repositorio en GitHub.

### 2021-01-13. 
* Plantear clases:
 * - [x] Fondo - **David Rojo y Dani Delgado**
 * - [x] Pieza - **David Rojo y Dani Delgado**
 * - [x] ControladorJuego - **David Rojo y Dani Delgado**
 * - [x] GdxGameSnake - **David Rojo y Dani Delgado**
 * - [x] Serpiente - **David Rojo y Dani Delgado**
 * - [ ] EstadoTeclado

Subimos fondo1.jpg - **David Rojo y Dani Delgado**

* ### 2021-01-19. 
	* *Daniel Mulero se ha unido al grupo.*

	* **David R:** Decidimos repartir las tareas en 2 subgrupos:

		1. **Equipo A**:
 			* Miguel Angel.
 			* Javier Sánchez.
 			* Dani Mulero.
 		2. **Equipo B**
 			* David Rojo.
 			* Dani Delgado.
 			* Alejandro Gomez.


> El equipo A se compromete a hacer commit de las nuevas funcionalidades cada jueves y el equipo B cada miercoles.

> El equipo A se encargarán de hacer los diagramas. El equipo B se encargarán de la clases.

## <span style="color:#fc038c"> 2021-01-19 TAREAS DE ESTA SEMANA:</span>

* Equipo A
	* - [x] Actualizar diagrama de clases. - **Javier S**
* Equipo B
	* - [ ] Plantear clase EstadoTeclado.
	* - [x] Añadir estado clase Pieza. - **David Rojo y Daniel Delgado.**
	* - [x] Añadir constructor clase Pieza. - **David Rojo y Daniel Delgado.**
	* - [x] Constructor copia de clase Pieza - **David Rojo y Daniel Delgado.**
	* - [x] Calcular colisiones clase Pieza. - **David Rojo y Daniel Delgado.**
	* - [x] Añadir Sprite de Serpiente. - **David Rojo y Daniel Delgado.**
	* - [ ] Conseguir resolución más pequeña de la pantalla
	* - [x] Mejorar estado de Serpiente. - **David Rojo y Daniel Delgado.**
	* - [x] Método moverse clase Serpiente. - **David Rojo y Daniel Delgado.**
	* - [x] Método dispose clase Serpiente. - **David Rojo y Daniel Delgado.**
	
* ### 2021-01-19. 
	* David R y Dani D: Modificaciones en el método colisiona de la clase Pieza.
	* Javier S. Me he encargado de desarrollar el diagrama UML(Pieza y Fondo con sus ESTADOS y COMPORTAMIENTOS).
* ### 2021-01-25. 
	* Miguel L., Dani M., Javier S. : Completar la clase fondo y actualizar UML con los nuevos métodos añadidos.
	* Miguel L., Dani M., Javier S. : Creamos la clase serpiente en el diagrama y planteamos su estado y algunos comportamientos.
* ### 2021-01-26. 
	* David R. Cambios en la clase Pieza y Serpiente.

## <span style="color:#fc038c"> 2021-01-28 TAREAS DE ESTA SEMANA: </span>

* Equipo A
	* - [x] Actualizar diagrama de clases. - Javier S
* Equipo B
	* Trabajo semana anterior:
		* - [ ] Plantear clase EstadoTeclado.
		* - [x] Conseguir resolución más pequeña de la pantalla  - **David Rojo y Daniel Delgado.**
	* - [x] Implementar clase Controlador. - **David Rojo y Daniel Delgado.**
	* - [x] Arreglar constructor copia de la clase Pieza. - **David Rojo y Daniel Delgado.**
	* - [x] Mejorar constructor Serpiente - **David Rojo y Daniel Delgado.**
	* - [ ] Añadir imágenes de inicio y fin al juego.
	* - [ ] Crear iniciaPartida finPartida.
	* - [x] Corregir métodos clase Pieza. - **David Rojo y Daniel Delgado.**
	* - [x] Hacer método crecer en Serpiente. - **David Rojo y Daniel Delgado.**


* ### 2021-02-02. 
	* Javier S. Subir capturas del desarrollo que ha tenido el diagrama UML
* ### 2021-02-03. 
	* David R.Cambios en clases: Pieza, Serpiente, ControladorJuego y GdxGameSnake

## <span style="color:#fc038c"> 2021-02-04 TAREAS DE ESTA SEMANA:</span>

* Equipo A
	* - [x] Crear diagrama de casos de uso. - Javier S
* Equipo B
	* Trabajo semana anterior:
		* - [x] Plantear clase EstadoTeclado. - **David Rojo y Daniel Delgado.**
		* - [ ] Añadir imágenes de inicio y fin al juego
		* - [x] Crear iniciaPartida finPartida. - **David Rojo y Daniel Delgado.**
	* - [x] Crear clase estadoTeclado. - **David Rojo y Daniel Delgado.**
	* - [ ] Arreglar métodos clase ControladorJuego
	* - [x] Método Serpiente chocar con cabeza. - **David Rojo y Daniel Delgado.**
	* - [x] Método Serpiente chocar con cuerpo. - **David Rojo y Daniel Delgado.**
	* - [x] Añadir constantes para imagenes de fin e inicio. - **David Rojo y Daniel Delgado.**
	* - [x] Añadir enum VideoJuego para controlar el estado del juego - **David Rojo y Daniel Delgado.**


* ### 2021-02-09. 
	* Javier S. Subir captura diagrama casos de uso.
* ### 2021-02-12. 
	* Daniel D, Alejandro G, David R. Cambios en las clases Pieza, Serpiente, ControladorJuego, GdxGameSnake, EstadoTeclado.

## <span style="color:#fc038c"> 2021-02-13 TAREAS DE ESTA SEMANA:</span>


* Equipo B
	* Trabajo semana anterior:
		* - [x] Arreglar métodos clase ControladorJuego. - **David Rojo y Daniel Delgado.**
		* - [x] Añadir imágenes de inicio y fin al juego. - **David Rojo y Daniel Delgado.**
	* - [x] Añadir EstadoTeclado al constructor del controlador. - **David Rojo y Daniel Delgado.**
	* - [x] Añadir método para controlar la partida. - **David Rojo y Daniel Delgado.**
	* - [ ] Añadir sonido en cada movimiento.
	* - [ ] Añadir musica de fondo.
	* - [x] Implementar o borrar clase Fondo. - **David Rojo y Daniel Delgado.**
	* - [x] Método de Serpiente para cambiar de dirección. - **David Rojo y Daniel Delgado.**
	* - [ ] Limpiar código.
	* - [x] Arreglar bug de doble movimiento. - **David Rojo y Daniel Delgado.**
	* - [x] Subir video haciendo el juego funcionar - **David Rojo y Daniel Delgado.**
* ### 2021-02-17. 
	* Daniel D. Diseño desde 0 de imagenes PixelArt para la pantalla de Inicio y Fin del juego(PROVISIONAL).
* ### 2021-02-17. 
	* David R, Alejandro G, Daniel D. Depuración de código, cambios en clases: Serpiente, ControladorJuego, GdxGameSnake, Pieza. Alcanzamos la primera versión jugable. El próximo objetivo es depurar pequeños errores y añadir un fondo
* ### 2021-02-17. 
	* David R. Mejoras en el diseño del README.
* ### 2021-02-18. 
	* Daniel D. cambios en las clases ControladorJuego, GdxGameSnake. Finalice el apartado artistico.
* ### 2021-02-18. 
	* David R. Arreglado bug doble movimiento al moverse y crecer en el mismo frame.
* ### 2021-02-19. 
	* Subimos el video demostrando que funciona en PC.

## <span style="color:#fc038c"> 2021-02-20 TAREAS DE ESTA SEMANA:</span>

* Equipo A
	* - [x] Actualizar diagrama de clases. - **Javier S, Dani Delgado, David Rojo**
	* - [x] Actualizar diagrama de objetos. - **Miguel L, Dani Delgado, David Rojo, Javier S**
* Equipo B
	* Trabajo semana anterior:
		* - [x] Añadir sonido en cada movimiento. - **David Rojo y Daniel Delgado.**
		* - [x] Añadir musica de fondo. - **David Rojo y Daniel Delgado.**
		* - [x] Limpiar código. - **David Rojo y Daniel Delgado.**
	* - [x] Añadir EstadoTeclado al constructor del controlador. - **David Rojo y Daniel Delgado.**
	* - [x] Arreglar bug colision fuera de pantalla. - **David Rojo y Daniel Delgado.**
	* - [ ] Arreglar bug de dirección al pulsar en la misma dirección.
	* - [ ] Hacer que funciona en horizontal y vertical.
* ### 2021-02-26. 
	* David R y Dani D. Añadimos música y efectos de sonido, fix de bug al colisionar fuera de la pantalla, depuración de código.
* ### 2021-02-26. 
	* David R y Dani D. Intento de arreglar bug de dirección y depuración de código. Eliminación de variables duplicadas y redundancia.
* ### 2021-02-27. 
	* David R. Subido video funcionando en Android en vertical y horizontal. Corrección bug no reproducir música de fin despues de la primera vez.
* ### 2021-02-27. 
	* Javier S. Subir Diagrama de clases UML completo con ayuda del equipo en las relaciones de clases.
* ### 2021-02-27. 
	* Miguel L., Javier S., David R., Dani D. Actualización diagrama de objetos.
* ### 2021-02-27. 
	* David R. Limpieza final de código. Listo para entregar.

## <span style="color:#fc038c"> 2021-02-28 TAREAS DE ESTA SEMANA:</span>

* Equipo A
	* - [x] Corregir errores diagrama casos de uso. - **Javier S**
	* - [x] Creación de diagrama de estado. - **Alejandro G y Javier S**
	* - [x] Crear diagrama de flujo. - **Miguel L, Daniel Mulero.**
* Equipo B
	* Trabajo semana anterior:
		* - [ ] Arreglar bug de dirección al pulsar en la misma dirección.
		* - [x] Hacer que funciona en horizontal y vertical. - **David Rojo y Daniel Delgado.**
	* - [x] Añadir EstadoTeclado al constructor del controlador - **David Rojo y Daniel Delgado.**
	* - [x] Arreglar bug colision fuera de pantalla. - **David Rojo y Daniel Delgado.**

### 2021-03-08.
Javier S. Correción de errores del diagrama de casos de uso.
### 2021-03-08.
Alejandro G. y Javier S. Creacion del diagrama de estados. Diagrama Inicio-Javier, Diagrama Durante Partida-Javier y Alejandro, Diagrama Fin-Alejandro
### 2021-03-08.
Daniel M. y Miguel L. Creacion del diagrama de flujo.

## <span style="color:#fc038c"> 2021-03-08 TAREAS DE ESTA SEMANA:</span>

* Equipo B
	* Trabajo semana anterior:
		* - [ ] Arreglar bug de dirección al pulsar en la misma dirección.
	* - [x] Arreglar readme. - **David Rojo y Daniel Delgado.**
	* - [x] Mostrar el reparto de tareas. - **David Rojo y Daniel Delgado.**
	* - [x] Mostrar propuesta de comienzo y final de partida. - **David Rojo y Daniel Delgado.**

### 09-03-2021
David R, Dani D. Arreglar readme de acuerdo a los requisitos del profesor.
